...
def pso_to_dataframe(pso, psodf):
    for i in np.arange(pso.shape[0]):
        pdf = psodf.iloc[i]
        ps_to_df(pso.iloc[i].long, pdf, 'L_')
        ps_to_df(pso.iloc[i].short, pdf, 'S_')
...
def get_daily_revenue(psodf, ic, trade_day):
    pass
...
def get_my_option(ds, instruments, trade_days, ic):
    ...
    psodf = init_psodf(ic)
    mmm.reset(200000000, 600000, 1.0, 0.0005)

    for i in np.arange(ds.shape[0]):
        row = ds.iloc[i]
        if trade_day < row.trade_day:
            print row.trade_day, " :"
            pso_to_dataframe(pso, psodf)
            get_daily_revenue(psodf, ic, trade_day)
