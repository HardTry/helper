...
def write_candle_to_mongo(datapath, datafile, instrument, period, format = 1):
    ...
    with open(datapath + '/' + datafile, "r") as tickcsv:
        ...
        for row in lines:
            ...
            if ok:
                try:
                  candledata.insert_one(candle.dict())
                except DuplicateKeyError:
                  print i, "duplicate key"
            else:
                print instrument, period, i, row
...
def open_monog(mongo_uri):
    ...
    candledata = mongodb.candledata


if __name__ == '__main__':
    if len(sys.argv) != 6:
        print "Usage tbk2mongo.py <datapath> <datafile> <mongo uri> <instrument> <period>"
    ...
    write_candle_to_mongo(datapath, datafile, instrument, period, 1)
