...
class SubplotAnimation(animation.TimedAnimation):
    def __init__(self, dm, trade, params, index, save_pos):
        self.index = index
        if len(dm) == 4:
            self.fig = plt.figure(index, figsize=(5, 4))
            self.ax = [self.fig.add_subplot(2, 2, 1),
                       self.fig.add_subplot(2, 2, 2),
                       self.fig.add_subplot(2, 2, 3),
                       self.fig.add_subplot(2, 2, 4)]
        elif len(dm) == 8:
            self.fig = plt.figure(index, figsize=(15, 7))
        ...
        for i in range(0, len(dm)):
            # self.art.append(artist.Artist(dm[i], self.ax[i], "art" + str(i)))
            self.art.append(artist.Artist10l(dm[i], self.ax[i]))
    ...
    def jump(self, pos):
        ...
        time.sleep(0.25)
        if (pos < 0 and self.params.curpos > 1024000 - pos) \
                or (pos > 0 and self.params.curpos
                    < self.params.min_data_size - pos):
            ...
            if pos > 10 or pos < 0:
                ...
                for i in range(0, len(self.dm)):
                    self.dm[i].ph.clear()
                    self.dm[i].do_math(self.params.curpos)
    ...
    def press(self, event):
        ...
        if event.key == 't':
            ...
            for i in range(0, len(self.dm)):
                print 'dm', self.params.curpos, i
                self.dm[i].print_values()
            print
        elif event.key == 'a':
            self.params.run_status = 0
            time.sleep(0.5)
    ...
    def _draw_frame(self, framedata):
        ...
        if self.cur_pos in self.stop_pos:
            self.params.run_status = 0
            timer.sleep(0.5)
            self.event_source.stop()
