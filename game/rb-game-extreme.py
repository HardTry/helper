...
class DataMath:
    ...
    def do_math(self, cp):
        ...
        self.ve = get_dfextreme(self.appx_lo, self.all_data[cp - self.data_len: cp], self.ve, cp - self.data_len)
        if (self.ve[-1].pos > self.first_ext_pos):
            self.new_ext = True
            self.ext_val = []
            self.ext_pos = []
            for e in self.ve:
                self.ext_pos.append(e.pos)
                self.ext_val.append(e.val)
            self.first_ext_pos = self.ve[-1].pos

        if self.new_ext:
            self.wt.setdata(self.all_data[self.first_ext_pos - self.data_len: self.first_ext_pos])
...
class Artist:
    ...
    def update_lines(self, xlim, cp, pp, its_review, show_future):
        ...
        if its_review:
            ...
            self.lNow.set_data([self.dm.first_ext_pos, cp], [self.dm.all_data[self.dm.first_ext_pos], self.dm.all_data[cp]])
    ...
    def update_extreme_lo(self, cp):
        # print self.name, cp, len(self.dm.cur_ext_pos)
        if self.dm.new_ext:
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) != 3):
        print 'Usage: python rb-game-extreme.py <r|the trade day> <data file path>\n' \
              ...
              '           python rb-game-extreme.py r /data/sean/20170508/bindata/rb888\n' \
              '         get one day data\n' \
              '           python rb-game-extreme.py 20150601 /data/sean/20170508/bindata/rb888\n\n' \
              '  Keyboard:                                \n' \
              '      i:	pause/continue                    \n' \
              '      a: 	open long                     \n' \
              '      b:	open short                        \n' \
