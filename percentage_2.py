    ...
    if is_open:
        ...
        if new_delta <= 0:
            ...
            if new_delta + mmm.my_deposit > mmm.max_dpst:
                print "can not open new position", trade
        ...
        if ps.vol < trade.volume:
            vdelta = ps.vol
            print "error: ", trade.inst, trade.trade_day
        else:
            vdelta = trade.volume
        ps.vol -= vdelta
...
def get_revenue(ps, price, is_all, is_long):
    rev = 0
    if is_all:
        rev = (ps.price - price) * ps.unit * ps.margin * ps.vol
    else:
        rev = (ps.myprice - price) * ps.unit * ps.margin * ps.myvol
...
def get_daily_revenue(pso, ic, trade_day):
    allrev = myall = 0
    for c in ic:
        ...
        if ps.long.vol > 0:
            candle = get_trade_day_finished_candle(c, trade_day)
            lrev = ps.long.rev + get_revenue(ps.long, candle.Close, True, True)
            mlrev = ps.long.myrev + get_revenue(ps.long, candle.Close, False, True)
        if ps.short.vol > 0:
            srev = ps.short.rev + get_revenue(ps.short, candle.Close, True, False)
            msrev = ps.short.myrev + get_revenue(ps.short, candle.Close, False, False)
    ...
    print trade_day, allrev, myall


def get_my_option(ds, instruments, trade_days, ic):
    ...
    for i in np.arange(ds.shape[0]):
        ...
        if trade_day < row.trade_day:
            ...
            #    pdf = psodf.loc[c]
            #    print c, pdf.to_json()
            get_daily_revenue(pso, ic, trade_day)
...
if __name__ == '__main__':
    ...
    cheack_finished_candle(ic, trade_days)
    mmm.reset(200000000, 1000000, 1.0, 0.0005)
    get_my_option(ds, instruments, trade_days, ic)
